...
function! s:CMakeSetCache(key, value)
    call util#SetCache(s:cache_cmake, getcwd(), a:key, a:value)
endfunction

function! s:CMakeGetCache(key, default)
    let l:val = util#GetCache(s:cache_cmake, getcwd(), a:key)
...
let s:cmake_last_executable = str2nr(s:CMakeGetCache(s:key_last_executable, '0'))
let s:cmake_run_args = s:CMakeGetCache(s:key_run_args, '')
...
function! CMakeSelectExecutable()
    " Select the executable target to run when CMakeRun is invoked.
    ...
    call s:CMakeSetCache(s:key_last_executable, l:select)

    " Clear arguments from previously selected executable
    call CMakeSetRunArguments('')
endfunction

function! CMakeSetRunArguments(args)
    " Set the arguments to pass to the executable when CMakeRun is called. An
    " empty string clears the arguments.
    "
    " Parameter: args string a space delimited string of arguments

    let s:cmake_run_args = a:args
    call s:CMakeSetCache(s:key_run_args, s:cmake_run_args)
endfunction

function! CMakeRun(...)
    " Invode the currently selected executable in the build directory. This
    " funciton passes the current run arguments to the executable.
    "
    ...
    if a:0 > 0
        call CMakeSetRunArguments(join(a:000, ' '))
    endif
...
function! CMakeBuildRun(target, ...)
    " Call CMakeBuild and on success call CMakeRun.
    "
    " Parameters:
    ...
    let l:build_error = CMakeBuild(a:target)
    ...
    let l:run_error = call('CMakeRun', a:000)
